  ...
  <div class="leiloes-page">
    ...
    <div v-if="telaAtual === 'lista'">
      ...
      <div class="cards-grid">
        <div v-if="leiloesComEstilo.length === 0" class="tabela-vazia">Nenhum leilão cadastrado.</div>

        <div
          v-for="leilao in leiloesComEstilo"
          :key="leilao.id_leilao"
          :class="['leilao-card', leilao.cardClass]"
        >
          <div class="card-header">
            <h3 class="card-name">{{ leilao.nome_evento }}</h3>
            <span :class="['badge', leilao.badgeClass]">{{ leilao.badgeText }}</span>
          </div>
          ...
          <div class="card-info">
            <p>
              ...
              {{ formatarDataBR(leilao.dt_leilao) }}
            </p>
            <p>
              <svg class="icon-sm" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="animais-vinculados">
            ...
            <div class="tags-container" v-if="leilao.animais && leilao.animais.length > 0">
              <span class="animal-tag" v-for="animal in leilao.animais" :key="animal.id">
                {{ animal.nome }}
              </span>
            </div>
    ...
    <!-- TELA DE FORMULÁRIO (CRIAÇÃO/EDIÇÃO) -->
...
// Computed: animais disponíveis para vincular (não ocupados por outro leilão)
...
// Computed principal: aplica as regras de estilo e badge para cada leilão
const leiloesComEstilo = computed(() => {
  ...
  return listaLeiloes.value.map(leilao => {
    let dataLeilao = null;
    if (leilao.dt_leilao) {
      dataLeilao = new Date(leilao.dt_leilao);
    ...
    if (!dataLeilao) {
      cardClass = 'card-agendado-futuro';
      badgeClass = 'badge-cinza';
      badgeText = '📅 Data inválida';
    } else if (dataLeilao < hoje) {
      // REALIZADO
      cardClass = 'card-realizado';
      badgeClass = 'badge-verde';
      badgeText = 'Realizado';
    } else {
      const diffDias = Math.ceil((dataLeilao - hoje) / (1000 * 60 * 60 * 24));
      if (diffDias <= 45) {
        // AGENDADO (até 45 dias)
        cardClass = 'card-agendado-proximo';
        badgeClass = 'badge-amarelo';
        badgeText = 'Agendado';
      } else {
        // POSSÍVEL LEILÃO (acima de 45 dias)
        cardClass = 'card-agendado-futuro';
        badgeClass = 'badge-cinza';
        badgeText = '📅 Possível Leilão';
      }
    }
...
.btn-excluir-icon:hover {
  background-color: #fef2f2;
}

.btn-excluir-icon svg {
  width: 20px;
  height: 20px;
}
